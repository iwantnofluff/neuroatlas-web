...
for cand in \
  ...
  "/Users/Jonathan/Downloads/NoFulff/NeuroAtlas/neuroatlas-web/.claude/gsd-core/bin/gsd-tools.cjs"; do
...
The `GSD_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`/Users/Jonathan/Downloads/NoFulff/NeuroAtlas/neuroatlas-web/.claude/`, `~/.gemini/`, `~/.codex/`, etc.), so the script invocation works for every runtime — not just Claude. An unresolved target exits in `get_installed_version` before this step.
...
global: `/Users/Jonathan/Downloads/NoFulff/NeuroAtlas/neuroatlas-web/.claude/`, `~/.config/opencode/`, `~/.opencode/`, `~/.gemini/`, `~/.config/kilo/`, or `~/.codex/`
