...
**`/gsd-capture --note <text>`** — Zero-friction timestamped note to `.planning/notes/` (or `/Users/Jonathan/Downloads/NoFulff/NeuroAtlas/neuroatlas-web/.claude/notes/` globally). Subcommands: append (default), list, promote (note → todo). Works without a project.
